...
subroutine region_condinit(x,q,dx,nn)
  ...
  real(dp),dimension(1:nvector,1:ndim)::x
   ...
   ! Initialize Wave region parameters
   real(dp) :: x_rel, y_rel, z_rel
   real(dp) :: lambda_x, lambda_y, lambda_z
   real(dp) :: amp, phi
   real(dp) :: rho0
   real(dp) :: p0
   real(dp), parameter :: pi = 3.141592653589793_dp

   ! Initialize Inverse region parameters
   real(dp) :: R_0, dist, a
  ...
  do k=1,nregion
     ...
     if(region_type(k) .eq. 'square')then
        ...
        do i=1,nn
           ...
           if(r<1.0)then
              q(i,1)=q(i,1)+d_region(k)
              q(i,2)=q(i,2)+u_region(k)
#if NDIM>1
              q(i,3)=q(i,3)+v_region(k)
#endif
#if NDIM>2
              q(i,4)=q(i,4)+w_region(k)
#endif
              q(i,ndim+2)=q(i,ndim+2)+p_region(k)
#if NENER>0
              do ivar=1,nener
                 q(i,ndim+2+ivar)=q(i,ndim+2+ivar)+prad_region(k,ivar)
              enddo
#endif
#if NVAR>NDIM+2+NENER
              do ivar=ndim+3+nener,nvar
                 q(i,ivar)=(i,ivar)+var_region(k,ivar-ndim-2-nener)
              end do
#endif
           end if
        end do
     ...
     ! Custom wave region
     if (region_type(k) .eq. 'wave') then
         
         ! Wave Parameters:
         ! Use v_region for amplitude
         amp = v_region(k)

         ! Use d_region for density
         rho0 = d_region(k)

         ! Use p_region for pressure
         p0 = p_region(k)

         ! use w_region variable for wavelength
         lambda_x = w_region(k)
         lambda_y = 0.0_dp
         lambda_z = 0.0_dp
 
         do i = 1, nn
            x_rel = x(i,1) - x_center(k)
#if NDIM>1
            y_rel = x(i,2) - y_center(k)
#else
            y_rel = 0.0_dp
#endif
#if NDIM>2
            z_rel = x(i,3) - z_center(k)
#else
            z_rel = 0.0_dp
#endif
            ...
            q(i,1) = q(i,1) + rho0 * (1.0_dp + amp * sin((x_rel * (2.0_dp * pi) / lambda_x) + phi))

            ! Pressure
            ! q(i,ndim+2) = q(i,ndim+2) + p0 * (q(i,1) / rho0) ** gamma
            q(i,ndim+2) = q(i,ndim+2) + p0
            ...
            ! print *, 'Density:', q(i,1)
            ! print *, 'xVel:', q(i,2)
            ! print *, 'Pressure:', q(i,ndim+2)
            ! print *, 'rho_old:', rho_old

         end do
      end if

      ! Custom triangular region
      if (region_type(k) .eq. 'triangle') then
         en = exp_region(k)
         do i = 1,nn
            ! Compute position in normalized coordinates
            xn = 0.0_dp
            yn = 0.0_dp
            zn = 0.0_dp
            xn = 2.0_dp * abs(x(i,1) - x_center(k)) / length_x(k)
#if NDIM>1
            yn = 2.0_dp * abs(x(i,2) - y_center(k)) / length_y(k)
#endif
#if NDIM>2
            zn = 2.0_dp * abs(x(i,3) - z_center(k)) / length_z(k)
#endif
            ! Compute cell "radius" relative to region center
            if (exp_region(k) < 10) then
               r = ((xn ** en) + (yn ** en) + (zn ** en)) ** (1.0_dp / en)
            else
               r = max(xn, yn, zn)
            end if

            ! Assign values to primitive variables
            if (r < 1.0) then
               q(i,1) = d_region(k) * (1.0_dp - r)
               q(i,2) = u_region(k)
#if NDIM>1
               q(i,3) = v_region(k)
#endif
               ...
               q(i,4) = w_region(k)
#endif
               q(i,ndim+2) = p_region(k) * (1.0_dp - r)
#if NENER>0
              do ivar = 1, nener
     ...
     ! Custom inverse region
      if (region_type(k) .eq. 'inverse') then
         do i = 1,nn
            ! Compute position in normalized coordinates
            xn = 0.0_dp
            yn = 0.0_dp
            ...
            zn = 2.0_dp * abs(x(i,3) - z_center(k)) / length_z(k)
#endif
            r = ((xn ** 2.0_dp) + (yn ** 2.0_dp) + (zn ** 2.0_dp)) ** (1.0_dp / 2.0_dp)

            R_0 = 0.5_dp * length_x(k)

            if (exp_region(k) == 0.0) then
               a = dx
            else 
               a = exp_region(k)
            endif
            ...
            if (r < 1.0) then
               ...
               dist = (a ** 2) * (R_0 ** 2) * (1.0_dp - (r ** 2)) / (((r ** 2) * (R_0 ** 2) + (a ** 2)) * ((R_0 ** 2) + (a ** 2)))

               q(i,1) = q(i,1) + d_region(k) * dist
               q(i,2) = q(i,2) + u_region(k)
#if NDIM>1
               q(i,3) = q(i,3) + v_region(k)
#endif
               ...
               q(i,4) = q(i,4) + w_region(k)
#endif
               q(i,ndim+2) = q(i,ndim+2) + p_region(k) * dist
#if NENER>0
              do ivar = 1, nener
                  q(i,ndim+2+ivar) = q(i,ndim+2+ivar) + prad_region(k,ivar)
              end do
